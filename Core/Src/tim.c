...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == TIM5 && HAL_GPIO_ReadPin(Button_GPIO_Port, Button_Pin) == GPIO_PIN_RESET)
    {
        ...
        {
            switch (audio_proc_get_display_mode())
            {
                case DISPLAY_FFT:
                {
                    audio_proc_set_display_mode(DISPLAY_HP_DATA);
                    break;
                }
                case DISPLAY_HP_DATA:
                {
                    audio_proc_set_display_mode(DISPLAY_MIC_DATA);
                    break;
                }
                case DISPLAY_MIC_DATA:
                {
                    audio_proc_set_display_mode(DISPLAY_FFT);
                    break;
                }
                default:
                {
